 ...
 */
class DemandeEtablissementRepository extends \Doctrine\ORM\EntityRepository
{ 
        ...
        public function findATraiterFini() 
        {
            $qb = $this->createQueryBuilder('demandes');
            $qb->where("demandes.etat=1");
            return $qb->getQuery()->getResult();
    ...
    public function nbParFiness($finess_code) 
    {
        $qb = $this->createQueryBuilder('demandes');
        $qb->select('count(demandes.id)');
        $qb->where("demandes.finess=:aaa");
        $qb->setParameter('aaa', $finess_code);
        return($qb->getQuery()->getSingleScalarResult());
    }
